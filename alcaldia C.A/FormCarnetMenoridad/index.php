...
$consulta_precio = "SELECT * FROM precios_doc where nombre_documento = 'partida_autentica_FormCarnet' ";
$row = mysqli_query($con,$consulta_precio);
$precio_doc_autentica = mysqli_fetch_assoc($row);
// precio partida
$consulta_precio = "SELECT * FROM precios_doc where nombre_documento =  'partida_normal_FormCarnet' ";
$row = mysqli_query($con,$consulta_precio);
...
<head>
    ...
    <title>Carnet de Menoridad</title>
    <!-- SweetAlert -->
...
<body>
    <div class="container">
        ...
        <h1 class="logo">Carnet de <span>Menoridad</span></h1>
            ...
            <div class="contact-form">
               ...
               <p name="info" class="infor" style="max-width:50%;width:auto;height:auto;">Normal: $<?php echo $precio_doc_patida['precio']  ?></p>
                <form id="datos" method="post" action="pagina.php">
                    <p>
                        <label for="names">Nombre del menor en la partida de nacimiento</label>
                        <input type="text" name="names" id="names" placeholder="Nombre completo" required>
                    </p>
                    <p>
                        <label for="surnames">Nombre del encargado (Padre, Madre u otro)</label>
                        <input type="text" name="encargado" id="surnames" placeholder="Nombre completo" required>
                    </p>
                    <p>
                        <label for="phone">DUI del encargado</label>
                        <input type="text" name="dui_encargado" id="dui" placeholder="00000000-0" pattern="^[0-9]{8}-[0-9]{1}$" required>
                        
                    </p>   
                    <p>
                    <label for="start">Dia de cita:</label>
                    <span class="fecha" id="date"></span><input type="text" name="cita" 
                    value="dd/mm/aaaa"
                      min="2022-01-01" max="2025-12-31" required></span>
                   </p>
                   <p class="">
                    <label for="surnames">Hora de cita</label>
